...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .setName('reason')
    .setDescription('The reason for locking')
    .setRequired(false));
const checkPermissions = (member) => {
    return (0, permissionChecker_1.checkCommandPermissions)(member, settings_json_1.default.commands?.lock, discord_js_1.PermissionFlagsBits.ManageChannels);
};
        ...
        locale = {
            commands: {
                lock: {
                    ...
                    success: "Channel {action} successfully",
...
exports.command = {
    name: 'lock',
    aliases: settings_json_1.default.commands?.lock?.aliases || [],
    enabled: settings_json_1.default.commands?.lock?.enabled ?? true,
    execute: async (interaction, args, client) => {
        ...
        try {
            ...
            if (currentPerms?.deny.has(discord_js_1.PermissionFlagsBits.SendMessages)) {
                const reply = { content: locale.commands.lock.alreadyLocked, flags: 1 << 6 };
                await (isSlash ? interaction.reply(reply) : interaction.reply(reply.content));
                return;
            }
            ...
            const embed = new discord_js_1.EmbedBuilder()
                ...
                value: locale.commands.lock.locked,
                inline: true
            }, {
